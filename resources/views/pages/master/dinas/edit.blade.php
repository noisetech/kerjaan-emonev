...
@section('content')
    ...
    <div class="container-fluid mt-3">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">Form ubah dinas</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
        ...
        <div class="card shadow mt-1">
            ...
            <div class="card-body">
                <form action="#" id="form_update_dinas" method="POST">
                    @csrf

                    <input type="hidden" name="id" id="id_dinas" value="{{ $dinas->id }}">
...
@push('script')
    <script>
        $("#form_update_dinas").submit(function(e) {
            e.preventDefault();
            const fd = new FormData(this);
            $.ajax({
                url: '{{ route('dinas.update') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
        ...
        $.ajax({
            type: 'GET',
            url: "{{ route('dinas.wilayahByDinas') }}",
            data: {
                id: $('#id_dinas').val(),
            }
        }).then(function(data) {
            ...
            for (i = 0; i < data.length; i++) {
                // selected
                var newOption = new Option(data[i].nama_wilayah, data[i].wilayah_id, true,
                    true);
        ...
        $.ajax({
            ...
            for (i = 0; i < data.length; i++) {
                // selected
                var newOption = new Option(data[i].users_email, data[i].users_id, true,
                    true);
